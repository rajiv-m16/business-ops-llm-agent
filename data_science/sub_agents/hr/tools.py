...
def get_filtered_schema(target_dataset_id: str, question: str = ""):
    ...
    # Fetch all columns and their semantic descriptions
    query = f"""
    ...
    # Filter if the schema is large and we have a question
    if len(df_schema) > 30 and question:
        # Simple keyword matching: escape keywords to prevent regex errors
        keywords = [re.escape(kw) for kw in question.lower().split()]
        pattern = '|'.join(keywords)
        
        # Keep rows where column_name OR description match user keywords
...
#     hr_dataset_id = "people_analytics" # Replace with your actual HR dataset
